...
class WSNDSPreprocessor(object):
    ...
    def visualize(self):
        """Visualize the data."""
        self.data["Attack type"].value_counts().plot.bar()  
        print(self.data)
    ...
    def train_valid_test_split(self):
        """"""
        self.labels = self.data['Attack type']
        self.features = self.data.drop(labels=['Attack type'], axis=1)
...
def WSNDS_processing(data_dir):
    ...
    wsnds.remove_duplicate_values()
    wsnds.remove_missing_values()
    wsnds.remove_infinite_values()

    # Drop constant & correlated features
    wsnds.remove_constant_features()
    # # print(cicids2017.data.info())    

    # Split & Normalise data sets
    training_set, validation_set, testing_set            = wsnds.train_valid_test_split()
    (X_train, y_train), (X_val, y_val), (X_test, y_test) = wsnds.scale(training_set, validation_set, testing_set)
...
if __name__ == "__main__":
    ...
    WSNDS_processing(args.data_dir)