...
def main(data_name):
    ...
    categorical_features_indices = X.columns.get_indexer(categorical_columns)
    X.index = pd.RangeIndex(0, len(X), step=1) 
    y.index = pd.RangeIndex(0, len(X), step=1)
    ...
    classifier.cross_validate((X,y), categorical_features_indices)
    # print(f"Useful feature: {selector.sel_col}")
