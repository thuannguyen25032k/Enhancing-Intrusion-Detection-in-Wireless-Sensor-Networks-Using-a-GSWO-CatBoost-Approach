...
def jfs(xtrain, ytrain, opts):
    ...
    # Divide the population into 3 groups including Exploitative sub-population, Exploratory sub-population, hesitate sub-population
    ...
    while t < max_iter:
        ...
        new_pop, index = np.unique(new_pop, return_index=True, axis=0)
        new_Fit = np.concatenate((fit, fit_new), axis=0)[index,:]
        new_pop = new_pop[new_Fit[:,0]<1,:]
        new_Fit = new_Fit[new_Fit[:,0]<1,:]
