...
def jfs(xtrain, ytrain, opts):
    ...
    # Calculate  Fitness at first iteration
    ...
    while t < max_iter:
        ...
        for i in range(Nc):
            ...
            P1      = X[k1,:].copy()    # Parent 1
            P2      = X[k2,:].copy()    # Parent 2
            # Random one dimension from 1 to dim
            index   = np.random.randint(low = 1, high = dim-1) # why
            # Crossover
        ...
        Xnew = np.concatenate((x1 , x2), axis=0)  # Why
        print(Xnew.shape)
        # Fitness
